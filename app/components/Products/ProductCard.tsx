...
// 🌄 Fallback image (si rien dans la DB)
...
const ProductCard: React.FC<ProductCardProps> = ({ product, className = "" }) => {
  ...
  const handleProductClick = () => router.push(`/products/${product.slug}`);
    ...
    >
      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-gray-50">
        ...
        <motion.button
          ...
          className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-blue-600 text-white px-4 py-2 rounded-full font-medium text-sm shadow-md flex items-center gap-2 transition-all duration-300 hover:bg-blue-700 active:scale-95 md:opacity-0 md:group-hover:opacity-100"
        >
          <Eye size={16} /> Voir le produit
        </motion.button>
      </div>
